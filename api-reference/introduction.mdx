...
```
https://{region-code}.wiresocket.net
```

Replace `{region-code}` with any available node region (e.g. `eu-central-1`). Any regional node can look up document locations in our global registry and return the correct destination.
