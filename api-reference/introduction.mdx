...
Replace `{region}` with any available region (e.g. `eu`). Document locations are stored in a global registry — any regional node can look up any document regardless of where it is hosted and return the correct node URL.
## Authentication
