...
WireSocket supports two connection strategies:
...
If Discovery is not used and a client connects to the wrong region, the node rejects the handshake with `4009`. Handle this as a transparent fallback:
```js
