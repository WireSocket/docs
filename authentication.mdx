...
The WireSocket dataplane verifies access token signatures using a tenant-scoped JWKS endpoint:
```
https://api.wiresocket.com/{tenantId}/.well-known/jwks
```

Replace `{tenantId}` with your tenant GUID, which is available in the `tenantid` claim of your JWT.

If you are building a custom dataplane integration or need to verify tokens server-side in your own infrastructure, use this endpoint to fetch the current public keys for your tenant.