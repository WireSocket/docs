...
| `dbCode`               | The specific database shard where your document metadata is stored |
...
WireSocket signs all access tokens using a **tenant-scoped RSA key pair**. If you need to verify token signatures server-side in your own infrastructure — for example, to validate a token independently without calling WireSocket APIs — you can fetch your tenant's current public keys from:

```
https://api.wiresocket.com/{tenantId}/.well-known/jwks
```

Replace `{tenantId}` with your tenant GUID, available in the `tenantid` claim of your JWT.

### Key Rotation & Multiple Keys

WireSocket automatically rotates your tenant's signing key on a **30-day cycle**. During rotation, the endpoint returns **up to two public keys**:

| Key         | Description                                               |
| ----------- | --------------------------------------------------------- |
| Active key  | Signs all new tokens                                      |
| Retired key | Previous key, valid for up to **24 hours** after rotation |
...
<Warning>
  ...
  Pinning to a single key will cause verification failures during the 24-hour
  rotation window.
</Warning>
