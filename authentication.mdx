...
<Warning>
  Validate against **all keys** returned in the response, not just the first. Pinning to a single key will cause verification failures during the 24-hour rotation window.
</Warning>

<Info>
  This endpoint is rate limited to **20 requests per 10 minutes**. Local caching is strongly recommended.
</Info>